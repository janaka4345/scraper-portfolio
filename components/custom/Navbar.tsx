    ...
    return (
        <nav className="bg-transparent bg-white h-[10svh] fixed w-full z-20 top-0 start-0 ">
            <div className=" flex flex-wrap h-full my-auto items-center justify-between mx-4 p-1">
                <a href="#" className="flex hover:scale-110 transition-all duration-150 items-center space-x-3 ">
                ...
                <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
                    <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border  md:space-x-8  md:flex-row md:mt-0 md:border-0  ">
                        {navMenu.map(item => (
                            <li key={item.item}>
                                <a href={item.url} className="block py-2 px-3  rounded md:bg-transparent text-primary md:p-0  hover:scale-110 transition-all duration-150" aria-current="page">{item.item}</a>
                            </li>
                        ))}
                    </ul>
                </div>
